...
sessiondatadir <- tibble(fname = list.files(l$datadir,paste0('data_exp_[^\\.]+Session.*$'),include.dirs = T) ) %>%
  filter(! str_detect(fname,'processed'),
         str_detect(fname,paste0('_',ExperimentName,'_')))


fs <- sapply(sessiondatadir,function(x){list.files(path=file.path(l$datadir,x),pattern='S._Welcome.csv|S4_ParticipantTracker.csv',full.names = T)})
...
fs <- sapply(sessiondatadir,function(x){list.files(path=file.path(l$datadir,x),pattern='S._.*.csv',full.names = T)})
