...
for (f in fs) {
  ...
  write_csv(d, file = file.path(sessiondatadir[str_detect(sessiondatadir,eid)], paste0(CurrNode$UniqueName, '_v-', unique(d$`Experiment Version`), '.csv')))
...
if ( !file.exists(f <- file.path(sessiondatadir,'extraColumnNames.csv'))){
