...
```{r, message=F}
eid <- as.numeric(ExperimentID)
zipfiles <- unlist(sapply(ExperimentID,function(f){list.files(params$rootdir,paste0('data_exp_',f,'-v.*.zip'),full.names = T)}))
...
Now that we have these, we know that the current data comes from experiment ``r ExperimentID``  (``r ExperimentName``). We can rename all files using `UniqueName` values from [this google sheet](https://docs.google.com/spreadsheets/d/1Mwy2aGCJ6vSpp4a32NOs83e2H73MQRFUOL_193yb8sQ/edit#gid=0).
...
for (f in fs) {
  # if (grepl('v43.*fth3',f)) {stop()}
  if (file.size(f) < 400) {
    file.remove(f)
    next
    }
  d <- switch(tools::file_ext(f),
              csv = read_csv(f,col_types = cols(.default = col_character())),
  ...
  CurrNode <- filter(allnodes,Session == globalenv()$Session,get(ExperimentName) == gsub('.*-([[:alnum:]]{4})','\\1',unique(d$`Tree Node Key`)))
