...
# list files
zipfiles <- unlist(sapply(ExperimentID,function(f){list.files(rootdir,paste0('data_exp_',f,'-v.*.zip'),full.names = T)}))
...
for (S in Session) {
  ...
  datadir[S] <- file.path(rootdir,paste0('data_exp_',paste(as.character(m$ExperimentID),collapse = '-'), '_', m$ExperimentName[1], '_Session',S))
}
...
for (f in fs) {
  ...
  write_csv(d, file = file.path(datadir[str_detect(datadir,eid)], paste0(CurrNode$UniqueName, '_v-', unique(d$`Experiment Version`), '.csv')))
...
if ( !file.exists(f <- file.path(rootdir,'extraColumnNames.csv'))){
