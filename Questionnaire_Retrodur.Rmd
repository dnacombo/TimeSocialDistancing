...
save(RetroDuration_clean, file = file.path(params$datadir,'RetroDuration_clean.RData'))
...
m <- lmer(log10(rEstimatedDuration) ~ Stringency_Index + Subjective_Confinement + Age + poly(Hour_Of_Day,3) +  (1 | PID), data = tostat )
summary(m)
anova(m)
...
```{r}
m <- lmer(log10(rEstimatedDuration) ~ Stringency_Index + Mobility_Transit + Subjective_Confinement + Age  +  (1 | PID), data = tostat )
summary(m)
anova(m)
effectsize::eta_squared(m)
```
