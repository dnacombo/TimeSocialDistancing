...
orig <- gimmedata(params$rootdir, ExperimentName = params$ExperimentName, UniqueName = 'Implicit', verbose = F) %>%
  mutate(ExperimentName = params$ExperimentName,
         `Reaction Time` = as.numeric(`Reaction Time`))

(N <- orig %>% filter(`Trial Number` %in% 'END TASK') %>%
    select(PID,Session,Run,UniqueName,`Repeat Key`) %>%
    group_by(Session,Run,UniqueName) %>%
    distinct() %>%
    summarize(N = n()))
...
(data_in_shape <- data_duration_filtered %>%
   ...
   select(`Participant Private ID`,PID,`Local Date`,
...
em <- emmeans(m1,~ Session + SOA + condition)
plot(pred)
...
```{r}
# data_all_exp <- gimmedata(params$rootdir, UniqueName = 'Implicit', verbose = F) %>%
data_all_exp <- data_all_exp %>% group_by(`Experiment ID`) %>%
  mutate(Country = ID2Country(ExperimentID = `Experiment ID`, ExperimentIDs = ExperimentIDs),
         `Reaction Time` = as.numeric(`Reaction Time`),
         PID = ifelse(is.na(PID),`Participant Private ID`, PID)) %>%
  select(ExperimentName, Session, UniqueName, Run, PID, everything())

```

```{r}
(N <- data_all_exp %>% filter(`Trial Number` %in% 'END TASK') %>%
   select(PID,Session,Run,UniqueName,`Repeat Key`, Country) %>%
   group_by(Session,Run,UniqueName, Country) %>%
   distinct() %>%
   summarize(N = n()) %>% ungroup() %>%
   select(Country,Session, Run, N) %>%
   arrange(Country, Session, Run)) %>%
  ggplot(aes(x=Country, y=N, fill = Session)) +
