...
for (f in fs) {
  ...
  dd <- d %>%
    ...
    # mutate(
    #   Question = str_replace(Question,'BFI-10_[[:digit:]]+','BFI-10'),
    #   Question = str_replace(Question,'FFA_[[:digit:]]+','FFA'),
    #   Question = str_replace(Question,'(HADS_[AD]_[[:digit:]]{2})-[[:digit:]]','\\1'),
    #   Question = str_replace(Question,'(OSRN)-[[:digit:]]{1,2}','\\1'),
    #   # Question = str_replace(Question,'(PSQI_11)[abcd]','\\1a-d')),
    #   # Question = str_replace(Question,'(PSQI_5)[abcdefghi]','\\1a-i'),
    #   # Question = str_replace(Question,'(PSQI_)[78]','\\17-8'),
    # ) %>%
...
Responses %>% group_by(UniqueName, Question) %>% group_keys() %>% left_join(TranslateOrMap,by=c('UniqueName','Question')) %>%
  write_csv(file.path(params$rootdir,'_TranslateOrMap.csv'))

# Demographics <- filter(Responses,UniqueName == "S1_Demographics")

splitResponses <- Responses %>%
  # filter(UniqueName != "S1_Demographics") %>%
  filter(is.na(as.numeric(Response))) %>% # keeping only responses that are not numeric
  group_by(UniqueName,Question) %>%
  group_split()
...
# }
# Demographics %>%
#   mutate(TranslatedResponse = NA) %>%
#   pivot_longer(matches('[[:digit:]]{5}_.*'),names_to = 'experimentID',values_to = 'Response') %>%
#   filter(!is.na(Response)) %>%
#   select(experimentID, UniqueName, Question,Response, TranslatedResponse, everything()) %>%
#   arrange(experimentID) %>%
  # write_csv(f <- file.path(params$rootdir,'ToTranslateSheetDemographics.csv'),na='')
```
