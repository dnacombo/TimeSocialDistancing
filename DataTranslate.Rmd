...
# For HADS questionnaire, we need to reorder some of the question labels. The translation sheet is found online.
HADS_exception <- gsheet2tbl('https://docs.google.com/spreadsheets/d/1KYTDeW_RacsZcPK-fMwKz5rw9dmxKQW4S1L6l8CeJg0/edit#gid=0')
```

### For questionnaires

```{r questionnaires, warning=F}
...
fs <- list.files(dataDir,pattern='(S[[:digit:]]+_[^\\/]*).csv',full.names = T,recursive = T) %>%
  tibble(fname=.) %>%
  ...
  # filter(UniqueName == 'ConfinementTrack') %>%
  group_by(UniqueName, experimentID) %>%
  group_split()

# allQ <- read_csv(f <- file.path(dataDir,'AllQuestionnaireQuestions.csv')) %>%
...
QTranslateOrMap <- Responses %>% 
  ...
  write_csv(file.path(dataDir,'_QTranslateOrMap.csv'),na = '')
...
splitResponses <- Responses %>%
  filter(is.na(suppressWarnings(as.numeric(Response))), # keeping only responses that are not numeric
          # but discard responses that match an expected pattern
         UniqueName != "RetroDuration" | (UniqueName == "RetroDuration" & !str_detect(Response,'^.*?(\\d+).*?[:：].*?(\\d+).*?[:：].*?(\\d+).*?$'))) %>%
...
for (r in splitResponses) {
  ...
  # merge experimentIDs to countries
  r <- mutate(r,Country = str_match(experimentID,'.*_([\\w-]+)$')[,2], .before = 1)
  # For each Country (i.e. language), list all unique Responses
  cts <- unique(r$Country)
  
  alluniques <- list()
  for (i in cts) {
    alluniques[[i]] = unique(filter(r,Country == i)$Response)
  }
  # count how many different response in each Country (this is just to pad lists across Countries)
  ls <- sapply(alluniques,length)
  
  # now list responses in wide format with columns == Country
  alluniques2 <- tibble(UniqueName = r$UniqueName[1], Question=r$Question[1], `EN-GB` = character(length = max(ls)), .rows = max(ls))
  for (i in cts) {
    alluniques2 <- mutate( alluniques2, !!i := c(sort(unique(filter(r,Country == i)$Response)), rep(NA,max(ls) - ls[i])))
  }
...
ToMap %>% select(UniqueName, Question, `EN-GB`,everything()) %>%
  write_csv(f <- file.path(dataDir,'ToMapSheetQuestionnaire_new.csv'),na='')
...
toto <- ToTranslate %>% 
  ...
  arrange(Country, UniqueName, Question, Response) %>%
  distinct(Country, UniqueName, Question, Response, .keep_all = T) %>%
  select(Country,UniqueName,Question,Response,Translated) %>%
  write_csv(f <- file.path(dataDir,'ToTranslateSheetQuestionnaire_new.csv'),na='')
...
TTranslateOrMap <- ResponseFields %>% 
  ...
  write_csv(file.path(dataDir,'_TTranslateOrMap.csv'),na = '')
...
ToMap %>%
  ...
  write_csv(file.path(dataDir,'ToMapSheetTask_new.csv'),na = '')
...
ToTranslate %>%
  ...
  distinct() %>%
  write_csv(file.path(dataDir,'ToTranslateSheetTask_new.csv'),na='')
