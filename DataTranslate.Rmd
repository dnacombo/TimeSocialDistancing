...
fs <- list.files(params$rootdir,pattern='(S1_[^\\/]*).csv',full.names = T,recursive = T) %>%
  tibble(fname=.) %>%
...
QTranslateOrMap <- Responses %>% 
  ...
  write_csv(file.path(params$rootdir,'_QTranslateOrMap.csv'),na = '')
...
TTranslateOrMap <- Responses %>% 
  ...
  write_csv(file.path(params$rootdir,'_TTranslateOrMap.csv'),na = '')
