...
QTranslateOrMap <- gsheet::gsheet2tbl('https://docs.google.com/spreadsheets/d/1bwKj-ngDrHFVXpSD13l183FHsXoZu1HqQmz9ZtYROYM/edit#gid=1012544807')
...
QToTranslate <- gsheet::gsheet2tbl('https://docs.google.com/spreadsheets/d/1YOZ_3MMdo7ghgdIyhxgWqE8WYsB_wFihlEAVhODkz7Q/edit#gid=1845970270')
  ...
  for (d1 in dd) {
    w <- expand_grid(experimentID = d1$experimentID,
      ...
      filter(!is.na(Response),
             is.na(suppressWarnings(as.numeric(Response)))) %>% # keeping only responses that are not numeric
      arrange(Response) %>%
      bind_rows(w)
  }
    ...
    switch(TM,
           ...
           T = {
             ...
             ToTranslate <- Responses %>%
               ...
               pivot_longer(Fi, names_to = 'Field', values_to = 'Value') %>%
               drop_na() %>%
               distinct() %>%
