...
(d %>% select(-mainexperimentName) %>%
    ...
    write_csv(file.path(params$rootdir,'SubjectsCount.csv'), na='')
)
...
d <- read_csv(file.path(params$rootdir,'SubjectsCount.csv'),col_types = cols(taskName=col_factor(),.default = col_integer())) %>%
