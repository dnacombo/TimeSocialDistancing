...
```{r}
alldata <- gimmeRdata(DataDir = dirBlursday, fast = T, progress = F, as.list = T)
```
...
```{r}
fs <- list.files('/home/maximilien.chaumon/ownCloud/Lab/00-Projects/TimeSocialDistancing/DATA', pattern = '^S._.*csv', recursive = T, full.names = T)
...
counts <- all_endedtasks %>%
  group_by(PID, Session, Unique_Name, Run) %>%
  slice(1) %>%
  group_by(PID) %>%
  summarize(n = n()) 
...
counts %>% ggplot(aes(x = n)) +
  geom_histogram(binwidth = 1)
```
```{r}
all_endedtasks %>%
  filter(Session == 'S1', Run %in% c('1', '')) %>%
  group_by(Unique_Name) %>%
  summarize(N = n()) %>%
  filter(!Unique_Name %in% c('CFC-14','Continue', 'END')) %>%
  ggplot(aes(x = Unique_Name, y = N)) +
  geom_col() +
  coord_flip() + 
  scale_x_discrete(limits=rev) +
  xlab('Questionnaire / Task')
```
