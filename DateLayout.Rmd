---
title: "Data all sessions, FR data"
params:
  ExperimentName: FR
...
```{r}
alldata <- gimmedata(rootdir,ExperimentName = 'FR')
```

All subjects, tasks, and sessions.
...
allPIDs <- alldata %>% group_by(`Participant Private ID`) %>%
  ...
  datatable(options = list(pageLength = 100)) %>% print()
```
...
ggplot(toplot,aes(x=`UTC Date`, y=PID, col=UniqueName, shape = Session)) +
  ...
  geom_vline(aes(xintercept = Date),linetype = 2,data=sessionstart) +
  geom_vline(aes(xintercept = Date),linetype = 3,data=sessionend) +
