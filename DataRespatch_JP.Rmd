---
title: "Data all sessions, JP data"
params:
  ExperimentName: JP
...
# Fix PID
One subject seems to have entered two different PIDs: `125aea` for session1, and `125new` for sessions 2 and 3. We merge it.
...
(nuPIDs <- nuPIDs %>% filter(! PID %in% c('125eae','125new','[test00]')) %>%
  bind_rows(patch)) %>%
  select(-email,-PID) %>%
  distinct() %>%
  write_csv(file.path(params$rootdir,paste0('AllPID_',ExperimentName[1], '.csv')),na='')
